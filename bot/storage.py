...
def _default_data():
    return {
        ...
        "missed_setups":   [],    # virtual trades for setups that were rejected near-miss
    }
...
MISSED_SETUP_LIMIT = 500


def save_missed_setup(setup: dict, rejection_reason: str) -> str:
    data = load_data()
    missed = data.setdefault("missed_setups", [])
    msid = uuid.uuid4().hex[:10]
    missed.append({
        "id":          msid,
        "pair":        setup.get("pair", ""),
    ...
    if len(missed) > MISSED_SETUP_LIMIT:
        del missed[: len(missed) - MISSED_SETUP_LIMIT]
    save_data(data)
    return msid


def get_pending_missed_setups() -> list:
    data = load_data()
    return [m for m in data.get("missed_setups", []) if m.get("outcome") is None]


def update_missed_setup_outcome(msid: str, outcome: str):
    data = load_data()
    for m in data.get("missed_setups", []):
        if m["id"] == msid:
            m["outcome"]     = outcome
...
def get_missed_winner_stats(hours: int = 168) -> dict:
    data = load_data()
    ...
    total      = len(items)
    wins       = sum(1 for m in items if m.get("outcome") == "win")
    losses     = sum(1 for m in items if m.get("outcome") == "loss")
    pending    = sum(1 for m in items if m.get("outcome") is None)
    expired    = sum(1 for m in items if m.get("outcome") == "expired")
    by_reason  = {}
    for m in items:
        r = m.get("rejection", "unknown")
    ...
    return {
        "hours": hours, "total": total, "wins": wins, "losses": losses,
        "pending": pending, "expired": expired, "by_reason": by_reason,
    }
