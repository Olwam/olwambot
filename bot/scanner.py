...
def scan_pair_for_setup(symbol: str, timeframe: str = None) -> dict | None:
    ...
    # Soft fallback: if quote is missing/stale, use last candle close so a
    # valid SMC setup is not silently dropped purely due to feed lag.
    stale_q, stale_q_reason = is_quote_stale(sym, quote, now_utc)
    quote_fallback = False
    if stale_q:
        # Soft fallback: use the most recent candle close instead of dropping.
        # We already have `candles` from the earlier candle fetch above —
        # reuse it instead of re-fetching to avoid an extra API call.
        _fb_price = (candles[-1].get("close") if candles else None)
        if not _fb_price:
            save_rejection(sym, "stale_quote", stale_q_reason, source="scanner")
            print(f"  [Scanner] {sym} gated: {stale_q_reason}", flush=True)
            return None
        quote = {"price": float(_fb_price)}
        quote_fallback = True
        print(f"  [Scanner] {sym} stale quote — using last candle close "
              f"{_fb_price} as fallback", flush=True)
    ...
    # When BOTH higher timeframes are neutral, market is ranging. Switch to a
    # mean-reversion strategy: lower RR target, prefer eq-H/L sweep entries
    # near range boundaries instead of trend-following pullback entries.
    range_mode = bool(htf_neutral_penalty) and bool(htf2_neutral_penalty)
    if range_mode:
        print(f"  [Scanner] {sym} RANGE MODE — both HTFs neutral, "
              f"using mean-reversion targets (RR 1.2)", flush=True)
    ...
    if regime in ("range", "mixed") and not range_mode:
    ...
    # Enforce minimum RR — relaxed in range mode (mean-reversion targets are tighter)
    target_rr  = 1.2 if range_mode else 2.0
    floor_rr   = 1.0 if range_mode else 1.5
    min_rr     = max(rules.get("min_rr", 1.5), target_rr) if not range_mode else target_rr
    if rr < min_rr:
        if direction == "BUY":
            tp = entry_price + risk * target_rr
        else:
            tp = entry_price - risk * target_rr
    ...
    if rr < floor_rr:
        save_rejection(sym, "poor_rr",
                       f"{sym} RR {rr:.2f} < minimum {floor_rr} even after adjustment",
                       source="scanner")
        return None
    ...
    if confidence < effective_watch and not force_watch:
        save_rejection(sym, "low_confluence",
                       ...
                       source="scanner")
        # Track as a "missed winner" candidate — full SL/TP exist; if the
        # virtual trade hits TP first, we know the gate was too strict.
        try:
            save_missed_setup({
                "pair": sym, "timeframe": tf, "direction": direction,
                "entry": round(entry_price, 5), "stop_loss": round(sl, 5),
                "take_profit": round(tp, 5), "confidence": confidence,
                "rr": round(rr, 2),
            }, rejection_reason="low_confluence")
        except Exception as _me:
            print(f"  [Scanner] missed_setup save error: {_me}", flush=True)
        return None
