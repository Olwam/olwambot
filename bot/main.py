...
def cmd_start(message):
    ...
    if is_approved(uid) or is_admin(uid):
        admin_note = (
            ...
            "\n   /latencystats /rejectionstats /recentrejections /missedwinners"
...
@bot.message_handler(commands=["missedwinners"])
def cmd_missedwinners(message):
    if not is_admin(message.from_user.id):
        bot.reply_to(message, "❌ Admin only.")
        return
    from storage import get_missed_winner_stats
    parts = message.text.strip().split()
    hours = 168
    if len(parts) >= 2:
        try:
    ...
    s = get_missed_winner_stats(hours=hours)
    if s["total"] == 0:
        bot.reply_to(
            message,
            f"🎯 No rejected setups tracked in last {hours}h yet.\n"
            f"Near-miss setups (rejected for low confluence) are auto-tracked "
            f"and resolved like real signals."
        )
        return

    resolved = s["wins"] + s["losses"]
    win_rate = (s["wins"] / resolved * 100) if resolved else 0
    lines = [
        f"🎯 MISSED WINNERS REPORT (last {hours}h)\n",
        f"Tracked:   {s['total']}",
        f"  ✅ Wins:    {s['wins']}",
        f"  ❌ Losses:  {s['losses']}",
        f"  ⏳ Pending: {s['pending']}",
    ...
    if resolved:
        lines.append(f"\nHypothetical win rate: {win_rate:.1f}%")
        if win_rate >= 60:
            lines.append("⚠ Filters may be too strict — many rejected setups would have won.")
        elif win_rate <= 35:
            lines.append("✓ Filters working — most rejected setups would have lost.")
        else:
            lines.append("≈ Borderline edge on rejected setups.")

    if s["by_reason"]:
        lines.append("\nBy rejection reason:")
        for r, d in s["by_reason"].items():
            r_resolved = d["wins"] + d["losses"]
            wr = (d["wins"] / r_resolved * 100) if r_resolved else 0
            lines.append(
                f"  {r:<18} total={d['total']:>3}  "
                f"W={d['wins']} L={d['losses']}  ({wr:.0f}% wr)"
            )
